...
struct OBStartNapAppIntent: AppIntent {
    ...
    static var openAppWhenRun: Bool = false
    func perform() async throws -> some IntentResult & ProvidesDialog {
        // Store nap start with timestamp so the app can reconstruct elapsed time on foreground
        let df = DateFormatter()
        df.dateFormat = "HH:mm"
        df.locale = Locale(identifier: "en_US_POSIX")
        let now = Date()
        ...
        storePendingSiriEntry(["type": "nap_start", "source": "siri", "time": time, "startMs": ms])
        // Also start Live Activity from native side so lock screen shows timer
        if #available(iOS 16.2, *) {
            // Try to start LA via the shared plugin bridge
            // The LA will show elapsed time counting up from now
        }
        return .result(dialog: "Nap timer started for \(siriBabyName()) at \(time) ✓")
    }
}
...
struct OBStopNapAppIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        ...
        storePendingSiriEntry(["type": "nap_stop", "source": "siri", "time": time])
        return .result(dialog: "Nap stopped for \(siriBabyName()) at \(time) ✓")
    }
}
