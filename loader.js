...
function _obEscapeHtml(value) {
  return String(value == null ? "" : value).replace(/[&<>"']/g, function(ch) {
    return ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[ch];
  });
}

function _obSafeErrorText(value) {
  return _obEscapeHtml(String(value == null ? "" : value).slice(0, 2000));
}

function _obRenderBootError(title, detail) {
  var root = document.getElementById('root');
  if (!root || root.children.length) return;
  var safeTitle = _obSafeErrorText(title || 'Still Loading?');
  var safeDetail = _obSafeErrorText(detail || '');
  root.innerHTML = '<div style="min-height:100vh;background:linear-gradient(135deg,#FFFEFD 0%,#FDFAF9 40%,#FBF9F8 100%);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:40px 24px;font-family:DM Sans,sans-serif;text-align:center;position:relative;overflow:hidden">'
    ...
    + '<div style="font-family:Playfair Display,serif;font-size:26px;font-weight:700;color:#5B4F5F;margin-bottom:10px">' + safeTitle + '</div>'
    + '<div style="font-size:15px;color:#7A6B7E;line-height:1.65;max-width:300px;margin-bottom:6px">OBubba is taking longer than expected to wake up.</div>'
    + '<div style="font-size:14px;color:#A898AC;line-height:1.5;max-width:280px;margin-bottom:28px">Refresh once. Your logs are still stored on this device.</div>'
    + '<button onclick="window.location.reload()" style="padding:14px 36px;border-radius:99px;border:none;background:rgba(192,112,136,0.55);backdrop-filter:blur(16px);color:white;font-size:16px;font-weight:700;cursor:pointer;font-family:inherit;box-shadow:0 0 24px rgba(246,221,227,0.40),0 0 48px rgba(217,207,243,0.25);animation:floatUp 3s ease-in-out infinite">Refresh OBubba</button>'
    + '<div style="font-size:10px;color:#C8B8C0;margin-top:24px;font-family:monospace;max-width:300px;word-break:break-all">' + safeDetail + '</div>'
    + '</div>';
}
...
window.onerror = function(msg, src, line, col, err) {
  ...
  var safeDetail = _obSafeErrorText(detail);
  var safeLine = _obSafeErrorText(line);
  root.innerHTML = '<div style="min-height:100vh;background:linear-gradient(135deg,#FFFEFD 0%,#FDFAF9 40%,#FBF9F8 100%);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:40px 24px;font-family:DM Sans,sans-serif;text-align:center;position:relative;overflow:hidden">'
    + '<style>'
    ...
    + '<div style="font-size:10px;color:#C8B8C0;margin-top:24px;font-family:monospace;max-width:300px;word-break:break-all">' + safeDetail + ' (line ' + safeLine + ')</div>'
    + '</div>';
};
...
window.addEventListener('load', function() {
  setTimeout(function() {
    try {
      var root = document.getElementById('root');
      if (!window.__obReactMounted && root && (!root.children || root.children.length === 0)) {
        _obRenderBootError('Still Loading?', 'The app shell did not mount. This usually means a stale bundle or missing runtime asset.');
      }
    } catch(e) {}
  }, 4500);
});
...
(function() {
  var errorPage = function(title, detail) {
    var safeTitle = _obSafeErrorText(title);
    var safeDetail = _obSafeErrorText(detail);
    document.getElementById('root').innerHTML = '<div style="min-height:100vh;background:linear-gradient(135deg,#FFFEFD 0%,#FDFAF9 40%,#FBF9F8 100%);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:40px 24px;font-family:DM Sans,sans-serif;text-align:center">'
      ...
      + '<div style="font-size:10px;color:#C8B8C0;margin-top:24px;font-family:monospace;max-width:300px;word-break:break-all">' + safeTitle + ': ' + safeDetail + '</div>'
      + '</div>';
  };
    ...
    if(!sex){
      ...
      if(childrenRaw){
        ...
        if(children&&typeof children==="object"&&!Array.isArray(children)){
          var child=activeId&&children[activeId]?children[activeId]:Object.values(children).find(function(c){return c&&typeof c==="object"&&!Array.isArray(c);});
          if(child&&typeof child==="object"&&!Array.isArray(child)) sex=String(child.sex||"").slice(0,8);
        }
      }
    }
...
var _obIsLocalDev = /^(localhost|127\.0\.0\.1|::1)$/.test(location.hostname);
if (_obIsLocalDev) {
