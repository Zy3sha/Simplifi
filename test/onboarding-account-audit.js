...
assert("onboarding saves the current Clock Track default instead of asking tired parents to choose a mode", app.includes('const[dashboardMode,setDashboardMode]') && app.includes('function setTrackModePreference(mode, opts = {})') && onboardingBlock.includes('setTrackModePreference("full")') && onboardingBlock.includes('dashboard_mode: "clock"') && !onboardingBlock.includes('data-testid="onboarding-dashboard-mode-choice"') && !onboardingBlock.includes("Choose your\\nTrack view"));
assert("new account child setup uses Clock Track and only keeps the current Track switch affordance", childSetupBlock.includes('setTrackModePreference("full")') && app.includes('data-testid="switch-to-full-dashboard"') && !app.includes('data-testid="switch-to-no-pressure"') && !app.includes('data-testid="track-mode-preference"') && app.includes('localStorage.setItem("ob_dashboard_mode_v1", next)'));
