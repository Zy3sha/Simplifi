...
assert("onboarding hero and baby-name step expose existing-account sign-in", onboardingBlock.includes('data-testid="welcome-sign-in"') && onboardingBlock.includes("I already have an account") && onboardingBlock.includes('setAuthMode("login")') && onboardingBlock.includes('setAuthScreen("login")'));
assert("welcome import flow guides parents through setup before CSV import and account creation", onboardingBlock.includes('data-testid="welcome-import-data"') && onboardingBlock.includes('ob_welcome_import_after_onboarding_v1') && onboardingBlock.includes('setShowImportAfterSetup(true)') && app.includes('data-testid="post-import-create-account"') && app.includes("Create free account") && app.includes('setAuthMode("create")') && app.includes('setAuthScreen("login")'));
...
assert("fresh onboarding and new-account setup queue the highlighted app tour", app.includes('localStorage.setItem("ob_app_tour_pending_v1"') && app.includes("function queueAppTourAfterSetup(reason)") && onboardingBlock.includes('queueAppTourAfterSetup("onboarding")') && childSetupBlock.includes('queueAppTourAfterSetup("new_account")') && app.includes('queueAppTourAfterSetup("partner_sync")') && app.includes("const APP_TOUR_TARGETS") && app.includes('className="ob-tour-spotlight"'));
assert("app tour includes OBubba care journey and Bubba Care lock guidance", app.includes("const OB_APP_TOUR_JOURNEY") && app.includes("Your OBubba care journey") && app.includes("Start OBubba tour") && app.includes('title:"Share the care"') && app.includes('["family-hub", "nav-settings"]') && app.includes('data-tour-id="family-hub"') && app.includes("Lock Bubba Care session when you need to close the carer portal") && app.includes('setTab("settings")'));
...
assert("account guide replay buttons launch tours without collapsing the guides row", app.includes('data-testid="replay-app-tour"') && app.includes('data-testid="replay-track-tour"') && app.includes("e.preventDefault();e.stopPropagation();setTab(\"day\");setDaySubScreen(null);setTutStep(0)") && app.includes("e.preventDefault();e.stopPropagation();setTab(\"day\");setDaySubScreen(null);setDayTutStep(0)"));
assert("onboarding explains and saves the persistent Track mode choice", app.includes('const[dashboardMode,setDashboardMode]') && app.includes('const[obDashboardModeChoice,setObDashboardModeChoice]') && app.includes('function setTrackModePreference(mode, opts = {})') && onboardingBlock.includes('data-testid="onboarding-dashboard-mode-choice"') && onboardingBlock.includes('title:"Choose your\\nTrack view"') && onboardingBlock.includes("Best if you mostly need to tap fast") && onboardingBlock.includes("Best if you want the whole day visible") && onboardingBlock.includes('setTrackModePreference(obDashboardModeChoice)') && onboardingBlock.includes('dashboard_mode: obDashboardModeChoice || "simple"'));
assert("new account child setup chooses Track mode and both Track UIs can switch quickly", app.includes('data-testid="child-setup-dashboard-mode-choice"') && childSetupBlock.includes("setTrackModePreference(obDashboardModeChoice)") && app.includes('data-testid="switch-to-full-dashboard"') && (app.includes('data-testid="switch-to-no-pressure"') || app.includes('data-testid={tab.id==="easy"?"switch-to-no-pressure":undefined}')) && !app.includes('data-testid="track-mode-preference"') && app.includes('localStorage.setItem("ob_dashboard_mode_v1", next)'));
