...
assert("app tour explains the onboarding help journey and Bubba Care handoff", app.includes("const OB_APP_TOUR_JOURNEY") && app.includes("Start with ordinary logging") && app.includes("For the first 3 days, just log life as it happens") && app.includes("OBubba learns your baby's rhythm and starts predicting naps and bedtime.") && app.includes("Look at Guidance and Insights when you want clarification") && app.includes('title:"Build your village"') && app.includes("Partner Sync keeps both parents or caregivers on the same live profile") && app.includes("Bubba Care creates a simple live care guide") && app.includes('title:"Focus on yourself too"') && app.includes("Parent Room is your space for breathing, check-ins and support.") && app.includes("Start OBubba tour") && app.includes('["family-hub", "nav-settings"]') && app.includes('data-tour-id="family-hub"') && app.includes("Lock Bubba Care session") && app.includes('setTab("settings")'));
assert("Family Hub child sharing keeps access first and invite details tucked away", app.includes('data-testid="child-sync-manage-access-card"') && app.includes("People with access") && app.includes("They can see and add logs for {child.name||\"this child\"} only.") && app.includes("Per-child sharing.") && app.includes("Perfect for blended families and co-parents who each have other children.") && app.includes("Invite someone") && app.includes("Send invite") && app.includes("setShowInviteDetails(v=>!v)") && !app.includes("Show QR/code") && !app.includes("https://api.qrserver.com/v1/create-qr-code/?size=190x190"));
