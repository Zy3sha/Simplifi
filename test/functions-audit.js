...
assert("account auth callable functions are source-tracked", source.includes("exports.usernameStatus = onCall") && source.includes("exports.accountLogin = onCall") && source.includes("exports.providerAccountSignIn = onCall") && source.includes("exports.resetAccountPin = onCall") && source.includes("exports.accountSignInStatus = onCall") && source.includes("exports.repairAccountSignIn = onCall") && source.includes("exports.saveRecoveryEmail = onCall") && source.includes("exports.recoveryEmailLookup = onCall"));
...
assert("provider sign-in verifies JWTs server-side before linking accounts", source.includes("function verifyProviderIdentityToken") && source.includes("fetchProviderKeys(provider, config)") && source.includes('header.alg !== "RS256"') && source.includes("crypto.verify(") && source.includes("audienceMatches(payload.aud, allowedAudiences)") && source.includes("allowedIssuers.includes") && source.includes("Number(payload.exp) <= nowSeconds"));
assert("provider sign-in links only an owned or backup-verified OBubba account", source.includes('db.collection("account_providers").doc(id)') && source.includes("!userOwnsAccountData(accountData, uid) && !accountBackupMatches(accountData, backupCode)") && source.includes("providerSubjectHash: identity.subjectHash") && source.includes("linkedProviders: { [provider]: true }") && !source.includes("idToken:") && !source.includes("identityToken:"));
assert("account repair verifies backup code server-side before recreating sign-in", source.includes("exports.repairAccountSignIn = onCall") && source.includes("accountBackupMatches(data, backupCode)") && source.includes("Backup code does not match this username") && source.includes("backupCode: backupMatches && data && data.backupCode ? data.backupCode : backupCode") && source.includes('db.collection("uid_to_backup").doc(uid).set'));
