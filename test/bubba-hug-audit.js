...
assert("Bubba Hug full card waits for Parent Room", app.includes('bubbaHugReceived && tab === "day" && daySubScreen === "wellbeing"'));
...
assert("Clock Track presence turns worldwide active parents into anonymous stars or flowers", app.includes("const[clockPresenceParents,setClockPresenceParents]=useState([])") && app.includes('setDoc(doc(db, "bubba_presence", myId)') && app.includes('query(collection(db, "bubba_presence"), orderBy("lastSeenMs", "desc"), limit(40))') && app.includes("parents.slice(0, clockHomeLabTheme === \"day\" ? 6 : 8)") && app.includes("const clockPresenceGlyphs = (clockPresenceParents || []).map") && app.includes('className={"ob-clock-presence-glyph "+(clockLabIsDay?"is-flower":"is-star")') && app.includes("Send a Bubba Hug to a parent awake now") && styles.includes("@keyframes obClockPresenceTwinkle") && styles.includes("@keyframes obClockPresenceHeartbeat"));
