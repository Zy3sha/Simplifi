...
assert("Android timer service avoids partial wake locks flagged by Play vitals", androidManifest.includes('android.permission.WAKE_LOCK" tools:node="remove"') && !/WakeLock|PowerManager|PARTIAL_WAKE_LOCK|acquireWakeLock|releaseWakeLock/.test(androidTimerService));
assert("Android task removal does not restart foreground services from the background", androidTimerService.includes("Starting a new foreground service from task-removal/background") && !/onTaskRemoved[\\s\\S]*?startForegroundService/.test(androidTimerService));
assert("Android WebView cache clear is version-gated and deferred", mainActivity.includes("LAST_CACHE_CLEAR_VERSION") && mainActivity.includes("currentVersionCode()") && mainActivity.includes("clearWebViewCacheAfterAppUpdate(webView)") && mainActivity.includes("postDelayed") && !mainActivity.includes("webView.clearCache(true);\n                applyEdgeToEdgeInsets"));
assert("Android WebView has a branded startup background before HTML paints", mainActivity.includes("webView.setBackgroundResource(R.drawable.launch_background);"));
assert("Android StatusBar plugin is patched away from deprecated system-bar color APIs", statusBarPatch.includes("Android 15 edge-to-edge owns system bar colors") && statusBarPatch.includes("WindowCompat.setDecorFitsSystemWindows") && !/setStatusBarColor|getStatusBarColor|setNavigationBarColor|SYSTEM_UI_FLAG|WindowManager/.test(statusBarPatch.replace(/^-.*$/gm, "")));
