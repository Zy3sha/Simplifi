...
assert("native timer plugins expose current timer state for JS recovery", [iosLiveActivityPlugin, iosNestedLiveActivityPlugin].every(src => src.includes('CAPPluginMethod(name: "getCurrentTimer"') && src.includes("@objc func getCurrentTimer") && src.includes('"startTimeMs": startMs')) && androidTimerPlugin.includes("public void getCurrentTimer") && androidTimerPlugin.includes('ret.put("startTimeMs", startTimeMs)'));
