...
assert("connected Apple or Google sign-in is remembered and hides manual recovery prompts", app.includes('const[connectedAuthProviders,setConnectedAuthProviders]=useState(()=>readStoredAuthProviderIds())') && app.includes("function readStoredAuthProviderIds()") && app.includes("function refreshConnectedAuthProviders()") && app.includes("rememberConnectedAuthProviders([id]);") && app.includes('data-ob-provider-connected="1"') && app.includes('data-ob-provider-recovery-note="1"') && app.includes("providerButtons = providerOptions.filter(providerId => !connectedSet.has(providerId));") && app.includes("Recovery email and memorable word are only shown for username/PIN accounts."));
