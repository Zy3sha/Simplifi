...
assert("provider buttons use server-verified account linking instead of a placeholder toast", app.includes('callAccountFunction("providerAccountSignIn"') && app.includes("finishProviderAccountLogin(") && app.includes("window.OBNative?.appleSignIn") && app.includes("window.OBNative?.googleSignIn") && !app.includes("Apple sign-in is next. For this build"));
