...
function runSourceWiringSimulations() {
  ...
  assert("manual night wake catch-up saves to the selected bedtime day", appSource.includes("parents may be catching up on last night's wakes retroactively") && appSource.includes("const _selectedDay = safeDateKey(selDay) || _todayCal;") && appSource.includes("const _selectedHasBed = (d[_selectedDay]||[]).some(e=>e.type===\"sleep\"&&!e.night);") && appSource.includes("targetDay = _selectedHasBed ? _selectedDay : _prevHasBed ? _prevSelected : _selectedDay;") && appSource.includes('source: isPendingWakeDetailSave ? "bedtimer-night-wake" : "manual-night-wake"'));
	  ...
	  assert("active nap clock matches live timer from localStorage as well as React state", appSource.includes('const lsEntryId = (()=>{try{return localStorage.getItem("nap_entry_id") || "";}catch{return "";}})();') && appSource.includes('const lsStartT = (()=>{try{return localStorage.getItem("nap_startT") || "";}catch{return "";}})();') && appSource.includes('(lsEntryId && entry.id === lsEntryId)') && appSource.includes('(lsStartT && entry.start === lsStartT)'));
	  assert("forgotten nap prompt snoozes after Still napping instead of repeating on reload", appSource.includes('localStorage.getItem("ob_forgot_nap_snooze_until")') && appSource.includes('Date.now()+90*60*1000') && appSource.includes('Date.now()+30*60*1000') && appSource.includes('localStorage.removeItem("ob_forgot_nap_snooze_until")'));
	  assert("native timer state can reconnect an in-progress nap when JS state drops", appSource.includes('plugins.OBLiveActivity?.getCurrentTimer') && appSource.includes('plugins.OBTimerService?.getCurrentTimer') && appSource.includes('showToast("😴 Nap timer reconnected"') && appSource.includes('localStorage.setItem("nap_entry_id",entryId);') && appSource.includes('localStorage.removeItem("ob_nap_stopped_at");'));
	  assert("cancel nap timer clears recovered localStorage/native timer state", appSource.includes('const _storedNapOn = (()=>{const v=_ls("nap_on",""); return v==="1" || v==="true";})();') && appSource.includes('const _cancelEntryId = napEntryId || _ls("nap_entry_id","");') && appSource.includes("const _napStillActive = !!(napOn || _storedNapOn || _cancelEntryId || _cancelStartT);") && appSource.includes('localStorage.setItem("timer_mode_v1","prediction")') && appSource.includes('forcePushNapStopToCloud("nap_cancelled");'));
	  assert("set-as-ongoing nap edits restart the real entry-day timer instead of leaving timer incomplete", appSource.includes('Object.keys(days||{}).find(dk => (days[dk]||[]).some(x => x && x.id === entryId))') && appSource.includes('localStorage.setItem("nap_entry_id",entryId);localStorage.setItem("nap_start_day",timerDay);localStorage.setItem("timer_mode_v1","activeSleep")') && appSource.includes('_startLA({type:"sleep",babyName:babyName||"Baby",startTime:startMs,nextNap:"Nap "+_napNum});'));
	  assert("backdated nap start picker writes matching active entry id before rendering clock", appSource.includes('localStorage.setItem("nap_entry_id",entryId);localStorage.setItem("nap_start_day",timerDay);localStorage.setItem("timer_mode_v1","activeSleep")') && appSource.includes('const timerDay = safeDateKey(localDateStr(startDate)) || selDay || todayStr();'));
	  assert("long restorative naps reset sleep pressure before the next nap prediction", appSource.includes("const longNapReset = lastDur >= 150;") && appSource.includes("Math.min(45, Math.max(25, Math.round(excess * 0.35)))") && appSource.includes("long nap reset sleep pressure"));
  ...
  }
		  {
		    ...
		    const lateDay = tickNextEventSim({ hasBedtime: false, planPred: { napStart_min: 990 }, bedMins: 1200, napsDone: 2, lateDayNapCannotFit: true, fallbackBedMins: 1080 });
			    assert("clock tick promotes bedtime when a late-day remaining nap cannot safely fit", lateDay.nextEvent.type === "bed" && lateDay.nextEvent.timeMins === 1080 && lateDay.nextNapMins === null && lateDay.napsComplete === true && lateDay.napBedConflict === true);
				    ...
				    assert("Today's Plan skips template naps after a restorative long nap has met day sleep", appSource.includes("function getLongNapPlanPressureMeta") && appSource.includes("skipTemplateNaps: restorativeLongNap && daySleepMet") && appSource.includes("bridgeOnlyAfterLongNap") && appSource.includes("long nap cleared sleep pressure") && !appSource.includes("long nap reduced sleep debt"));
