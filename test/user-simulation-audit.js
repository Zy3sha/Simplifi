...
function runSourceWiringSimulations() {
  ...
  assert("late 4-hour evening nap blocks render as bedtime without rewriting the saved nap row", appSource.includes("const CLOCK_LATE_NAP_SLEEP_MIN_MINS = 240;") && appSource.includes("const clockIsLateLongNapSleepLab = (entry) =>") && appSource.includes("return start >= CLOCK_LATE_NAP_SLEEP_START_MINS || end >= CLOCK_LATE_NAP_SLEEP_END_MINS;") && appSource.includes('clockIsLateLongNapSleepLab(entry) ? "sleep"') && appSource.includes('if (clockIsLateLongNapSleepLab(entry)) return "Bedtime";') && appSource.includes('return clockLogJoinLab(duration ? hm(duration) : "", "from nap timer");') && appSource.includes('target:isLateLongNapSleep ? "sleep" : type'));
