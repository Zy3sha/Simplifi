...
assert("child sync writes are field allowlisted", rules.includes("function validChildSyncWrite()") && rules.includes("'participants'") && rules.includes("'deletedEntryIds', 'deletedDays'"));
...
assert("same-backup child sync ownership can be reclaimed after uid rotation", rules.includes("function childSyncBackupOwnerClaimWrite(code)") && rules.includes("uid_to_backup/$(resource.data.ownerUid)") && rules.includes("childSyncBackupOwnerClaimWrite(code)") && rules.includes("request.resource.data.child == resource.data.child"));
...
assert("app can reclaim child-sync ownership after same-account uid rotation", app.includes("async function claimChildSyncBackupOwner(") && app.includes("const claimedOwner = await claimChildSyncBackupOwner(childId, code, existingData);") && app.includes("if(!claimedOwner)"));
